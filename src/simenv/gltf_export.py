...
from typing import ByteString, List, Set, Tuple, Optional
...
def add_numpy_to_gltf(np_array: np.ndarray, gltf_model: gl.GLTFModel, buffer_data: bytearray, normalized: Optional[bool] = False, buffer_id: int=0) -> int:
    ...
    if (accessor_type == gl.AccessorType.MAT2 and component_type in [gl.ComponentType.BYTE, gl.ComponentType.UNSIGNED_BYTE]) or(
        accessor_type == gl.AccessorType.MAT3 and component_type in [gl.ComponentType.BYTE, gl.ComponentType.UNSIGNED_BYTE,
        gl.ComponentType.SHORT, gl.ComponentType.UNSIGNED_SHORT]):
        raise NotImplementedError("Column padding should be implemented in these cases."
    ...
    buffer_view_id = add_data_to_gltf(new_data=new_data, gltf_model=gltf_model, buffer_data=buffer_data, buffer_id=buffer_id)

    # Create and add a new Accessor
    accessor = gl.Accessor(bufferView=buffer_view_id, byteOffset=0, componentType=component_type.value, normalized=normalized,
        type=accessor_type.value, count=count, min=min, max=max, sparse=False)
...
def add_image_to_gltf(image: PIL.Image, gltf_model: gl.GLTFModel, buffer_data: bytearray, buffer_id: int=0) -> int:
    ...
    buffer_view_id = add_data_to_gltf(new_data=data, gltf_model=gltf_model, buffer_data=buffer_data, buffer_id=buffer_id)
...
def add_material_to_gltf(tm_material: trimesh.visual.material.PBRMaterial, gltf_model: gl.GLTFModel, buffer_data: bytearray, buffer_id: int=0) -> int:
    ...
    for key, image in images_to_add.items():
        ...
        if image is not None:
            texture_id = add_image_to_gltf(image=image, gltf_model=gltf_model, buffer_data=buffer_data, buffer_id=buffer_id)
    ...
    try:  # try to convert base color to (4,) float color
        pbr_metallic_roughness.baseColorFactor = trimesh.visual.color.to_float(tm_material.baseColorFactor).reshape(4).tolist()
...
def add_mesh_to_model(tm_mesh: trimesh.Trimesh, gltf_model: gl.GLTFModel, buffer_data: ByteString, buffer_id: int=0) -> int:
    ...
    primitive = gl.Primitive(
        mode=gl.PrimitiveMode.TRIANGLES.value,
        attributes=attributes)
    mesh = gl.Mesh(primitives=[primitive])

    # Store vertex positions
    np_array = tm_mesh.vertices.astype(NP_FLOAT32)
    attributes.POSITION = add_numpy_to_gltf(np_array=np_array, gltf_model=gltf_model, buffer_data=buffer_data, buffer_id=buffer_id)

    # store vertex normals (TODO maybe not always necessary?)
    np_array = tm_mesh.vertex_normals.astype(NP_FLOAT32)
    attributes.NORMAL = add_numpy_to_gltf(np_array=np_array, gltf_model=gltf_model, buffer_data=buffer_data, buffer_id=buffer_id)
    ...
    if vertex_colors is not None:
        ...
        attributes.COLOR_0 = add_numpy_to_gltf(np_array=np_array, gltf_model=gltf_model, buffer_data=buffer_data, buffer_id=buffer_id)

    # Store face indices
    np_array = tm_mesh.faces.astype(NP_UINT32)
    primitive.indices = add_numpy_to_gltf(np_array=np_array, gltf_model=gltf_model, buffer_data=buffer_data, buffer_id=buffer_id)

    # append the material and then set from returned index
    if hasattr(tm_mesh.visual, 'material'):
        material_id = add_material_to_gltf(tm_material=tm_mesh.visual.material, gltf_model=gltf_model, buffer_data=buffer_data, buffer_id=buffer_id)
        primitive.material = material_id

        # if tm_mesh has UV coordinates defined export them
        has_uv = (hasattr(tm_mesh.visual, 'uv') and
                  tm_mesh.visual.uv is not None and
                  len(tm_mesh.visual.uv) == len(tm_mesh.vertices))
        if has_uv:
            ...
            uv_accessor_id = add_numpy_to_gltf(np_array=np_array, gltf_model=gltf_model, buffer_data=buffer_data, buffer_id=buffer_id)
...
def add_camera_to_model(camera: Camera, gltf_model: gl.GLTFModel, buffer_data: ByteString, buffer_id: int=0) -> int:
    ...
    if camera.camera_type == "perspective":
        gl_camera.perspective = gl.PerspectiveCameraInfo(aspectRatio=camera.aspect_ratio, yfov=camera.yfov, zfar=camera.zfar, znear=camera.znear)
    else:
        gl_camera.orthographic = gl.OrthographicCameraInfo(xmag=camera.xmag, ymag=camera.ymag, zfar=camera.zfar, znear=camera.znear)
...
def add_node_to_scene(node: Asset, gltf_model: gl.GLTFModel, buffer_data: ByteString, gl_parent_node_id: Optional[int] = None, buffer_id: Optional[int]=0):
    ...
    if isinstance(node, Camera):
        gl_node.camera = add_camera_to_model(camera=node, gltf_model=gltf_model, buffer_data=buffer_data, buffer_id=buffer_id)
        ...
        gl_node.mesh = add_mesh_to_model(tm_mesh=node.mesh, gltf_model=gltf_model, buffer_data=buffer_data, buffer_id=buffer_id)
    ...
    for child_node in node.children:
        add_node_to_scene(node=child_node, gl_parent_node_id=gl_node_id, gltf_model=gltf_model, buffer_data=buffer_data, buffer_id=buffer_id)
