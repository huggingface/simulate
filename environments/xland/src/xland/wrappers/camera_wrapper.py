...
class CameraWrapper(gym.ObservationWrapper):
    ...
    def __init__(self, env, swap_dims=True, name="pixels", n_maps=1):
        ...
        if swap_dims:
            obs_shape = env.observation_space["camera"].shape
            self.observation_space = spaces.Box(
                low=0, high=255, shape=(obs_shape[1], obs_shape[2], obs_shape[0]), dtype="uint8"
            )

        else:
            self.observation_space = env.observation_space["camera"]
    ...
    def observation(self, obs):
        if self.n_maps > 1:
            return {"pixels": obs["camera"].transpose((0, 2, 3, 1))}
        return {"pixels": obs["camera"].transpose((1, 2, 0))}
