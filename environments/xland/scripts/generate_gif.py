...
from xland.world import generate_map, generate_tiles
...
def generate_gif_maps(args):
    ...
    if args.sample_from is not None:
        ...
        grid.plot(cmap='summer', show_scalar_bar=False, screenshot=os.path.join(args.folder_path, "screenshot", str(counter) + ".png"),
                    off_screen=True)
    ...
    for width in range(args.min_width, args.max_width, args.interval_width):
        for height in range(width, width + args.interval_width + 1, args.interval_width):
            ...
            generated_map, map_2d, scene = generate_map(width=width, height=height, periodic_output=args.periodic_output,
                sample_from=args.sample_from, seed=args.seed, max_height=args.max_height, N=args.N, 
                periodic_input=args.periodic_input, ground=args.ground, symmetry=args.symmetry)
