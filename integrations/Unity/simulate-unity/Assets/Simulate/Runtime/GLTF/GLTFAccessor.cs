// adapted from https://github.com/Siccity/GLTFUtility/blob/master/Scripts/Spec/GLTFAccessor.cs
...
namespace Simulate.GLTF {
    public class GLTFAccessor {
        ...
        public class ImportResult {
            const float byteNorm = 1f / byte.MaxValue;
            const float shortNorm = 1f / short.MaxValue;
            const float ushortNorm = 1f / ushort.MaxValue;
            const float intNorm = 1f / int.MaxValue;
            const float uintNorm = 1f / uint.MaxValue;
            ...
            public Func<BufferedBinaryReader, float> GetNormalizedFloatReader(GLType componentType) {
                ...
                switch (componentType) {
                    case GLType.BYTE:
                        return x => x.ReadSByte();
                    case GLType.UNSIGNED_BYTE:
                        return readMethod = x => x.ReadByte() * byteNorm;
                    case GLType.FLOAT:
                        return readMethod = x => x.ReadSingle();
                    case GLType.SHORT:
                        return readMethod = x => x.ReadInt16() * shortNorm;
                    case GLType.UNSIGNED_SHORT:
                        return readMethod = x => x.ReadUInt16() * ushortNorm;
                    case GLType.UNSIGNED_INT:
                        return readMethod = x => x.ReadUInt32() / uintNorm;
                    default:
                        Debug.LogWarning("No componentType defined");
                        return readMethod = x => x.ReadSingle();
                }
